    ...
    <div className='group w-full md:w-1/2 lg:w-1/3 p-2'>
      <div className="relative bg-white rounded-lg shadow-md overflow-hidden group-hover:bg-gray-200 transition-all duration-300 h-full flex flex-col">
        {menuItem.mediaType === 'image' ? (
          <img src={menuItem.mediaSrc} alt={menuItem.name} className="w-full h-48 object-cover object-center rounded-lg" />
        ) : (
          <video src={menuItem.mediaSrc} className="w-full h-48 object-cover object-center rounded-lg" autoPlay loop muted playsInline/>
        )}
